...
//         try {
//             const emtrfs = await EMPTRF.find({ employeeCode: req.session.user.userId });
//             console.log('Employee EMTRFs:', emtrfs); // Log fetched data
...
app.get('/employee/dashboard', async (req, res) => {
    if (req.session.user && req.session.user.role === 'employee') {
        ...
        const query = { employeeCode: req.session.user.userId };
...
app.get('/hod/dashboard', async (req, res) => {
    if (req.session.user && req.session.user.role === 'hod') {
        ...
        const query = { status: 'pending', hodId: req.session.user.userId };
        ...
        try {
            const emtrfs = await EMPTRF.find(query);
            console.log('HOD EMTRFs:', emtrfs); // Log fetched data
